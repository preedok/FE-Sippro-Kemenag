...
const CardPenilaian = () => {
  ...
  const getData = async () => {
    ...
    try {
      const response = await api.get(
        `/prodi-assesment-al/${programStudiId}/${asesorId}`,
        {
      ...
      const defaultQuestions = {
        form: [
          ...
          {
            code: "APSL",
            question: "Akreditasi Program Studi Linear",
            nilai: "",
            note: "",
          },
          {
            code: "KDT",
            question:
              "Kesiapan Dosen tetap (Khusus S2: Kesediaan minimal 2 Guru Besar)",
            nilai: "",
            note: "",
          },
          {
            code: "KTP",
            question: "Kesiapan Tenaga Pendidik",
            nilai: "",
            note: "",
          },
        ...
        kesimpulan: [
          {
            code: "TRDS",
            question: "Diterima/Direkomendasikan",
            nilai: "",
            note: "",
          },
          {
            code: "DSMSP",
            question: "Direkomendasikan setelah melengkapi syarat-syarat",
            nilai: "",
            note: "",
          },
          {
            code: "TDKTMSP",
            question: "Tidak Diterima/ditolak",
            nilai: "",
            note: "",
          },
      ...
      const questions =
        responseData?.asesorALProgramStudis?.questions || defaultQuestions;

      // Ensure 'form' and 'kesimpulan' properties exist
      questions.form = questions.form || defaultQuestions.form;
      questions.kesimpulan =
        questions.kesimpulan || defaultQuestions.kesimpulan;
  ...
  const handleInputChange = (e, index, propName) => {
    const newData = { ...data };
    const questions = [
      ...(newData.asesorALProgramStudis?.questions?.form || []),
    ];
    if (!questions[index]) {
      questions[index] = {};
    }
  ...
  const handleInputChangeNote = (e, index, propName) => {
    const newData = { ...data };
    const questions = [
      ...(newData.asesorALProgramStudis?.questions?.kesimpulan || []),
    ];
  ...
  const handlePutRequest = async () => {
    ...
    const payload = {
      ...
      assesmentAssignedTime:
        data?.asesorALProgramStudis?.assesmentAssignedTime ||
        new Date().toISOString(),
      statusStr: "Draft",
      status: 223,
      assesmentDurationInDays:
        data?.asesorALProgramStudis?.assesmentDurationInDays || 0,
      ...
      asesorProdiAkreditasi:
        data?.asesorALProgramStudis?.asesorProdiAkreditasi || "",
      uups: uupsValue || "",
  ...
  const handlePutRequestDraf = async () => {
    ...
    try {
      const response = await api.put(
        "/prodi-assesment-al/assesment/lapangan",
        payload,
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const [checkedStates, setCheckedStates] = useState(
    Array(data.asesorALProgramStudis.questions.form.length).fill(false)
  );
      ...
      <div className="card-body m-4">
        <div className="row mt-3">
          <h5 style={{ color: darkMode ? "white" : "" }}>
            Form Penilaian Lapangan Asesor
          </h5>
          <div className=" mt-3">
            <div
            ...
            >
              ...
              <h4 className="my-4" style={{ color: darkMode ? "white" : "" }}>
