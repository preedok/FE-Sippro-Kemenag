...
import FormPenilaianSarjana from "../../module/usulan-prodi/detail-penilaian/form-penilaian-sarjana/edit/editPenilaianAssesor";
import FormPenilaianDoktor from "../../module/usulan-prodi/detail-penilaian/form-penilaian-doctor/edit/editPenilaianAssesor";
import FormPenilaianMagister from "../../module/usulan-prodi/detail-penilaian/form-penilaian-magister/edit/editPenilaianAssesor";
import FormPenilaianProfesi from "../../module/usulan-prodi/detail-penilaian/form-penilaian-profesi/edit/editPenilaianAssesor";
...
const CardUsulan = () => {
  ...
  const getStatusTextExcel = (status) => {
    ...
    // Convert status to string to ensure proper lookup
  ...
  const exportToExcel = () => {
    ...
    const exportData = filteredData.map((row) => ({
      ...
      // Pastikan status dikonversi ke text dan digabung dengan AssesmentCount jika ada
  ...
  const handleApproveSKAndParaf = (id, DocumentDate, note, file, nik, password, documentNo) => {
    ...
    axios
      .post(
        `${baseUrl}/prodi/paraf1sk/${id}?DocumentDate=${DocumentDate}&documentNo=${documentNo}&nik=${nik}&password=${password}&note=${note}`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${token}`,
...
`,
  ...
  const handleApproveBANPT = async (file, nomorSurat, note) => {
    try {
      StartLoading();
      const formData = new FormData();
      formData.append("file", file);
      ...
      const response = await axios.post(`${baseUrl}/prodi/tovalidate`, formData, {
        headers: {
          'accept': '*/*',
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'multipart/form-data',
      ...
      console.error('Error in handleApproveBANPT:', error);

      let errorMessage = "Terjadi kesalahan";

      if (error.response) {
        if (error.response.status === 400) {
          if (error.response.data.errors && error.response.data.errors.selectedProdi) {
            errorMessage = error.response.data.errors.selectedProdi[0];
          } else if (error.response.data.message) {
            errorMessage = error.response.data.message;
          }
        } else {
          errorMessage = error.response.data.message || "Gagal Konfirmasi";
        }
                ...
                <div>
                  ...
                  <div>
                    ...
                    <MenuItem onClick={() => {
                      ...
                      {params.row.Status === 26 ? "Konfirmasi" : ''}
                    </MenuItem>
                  ...
                  itemComponent={
                    <>
                      ...
                      <MenuItem onClick={() => {
                        ...
                        } else if (row.Status === 223) {
                          handleOpenValid(params.row.Id);
                        }
                      }}>
                        {/* {params.row.Status === 223 ? "Konfirmasi Proses ke Lembaga Akreditasi" : ''} */}
            ...
            <DropdownAksi
              itemComponent={
                <div>
                  ...
                  <div>
                    <MenuItem
                      onClick={() => handleDetailClickAL(params.row.Id)}
                    >
                      Detail Asesment AL
                    </MenuItem>
                    <MenuItem
...
<textarea id="swal-textarea" class="swal2-textarea ms-5" placeholder="Konfirmasi dengan catatan" style="width: 76%; margin-top: 10px; display: none;"></textarea>
            ...
            <DropdownAksi
              itemComponent={
                <>
                  <MenuItem onClick={() => fetchProdiHistory(params.row.Id)}>Riwayat</MenuItem>
                  <Link
                    to={`/dokumen-kma-izin/${params?.row?.Id}`}
                    style={{
                      textDecoration: "none",
                      color: "inherit",
                    }}
                  ...
                  <Box sx={{ padding: 2, overflow: 'auto' }} id="riwayatModalContent">
                    ...
                    <div style={{ color: darkMode ? "white" : "", }} className="d-flex justify-content-between mb-3">
                      <div className="mt-4">
                        ...
                        <p className="fw-bold" style={{ marginTop: '-16px' }}>DIREKTORAT JENDRAL PENDIDIKAN ISLAM</p>
  ...
  const handleDetailClickAL = (id) => {
    setDetailModalOpenAL(true);
    setIsLoading(true)
    axios
      .get(`${baseUrl}/prodi-assesment-al/${id}`, {
        headers: {
      ...
      .then((response) => {
        const result = response.data.data;
        setDetailDataAL(result);
        console.log('al', result);
        if (result && Array.isArray(result) && result.length > 0) {
          const asesors = result[0].asesorALs;
          if (Array.isArray(asesors)) {
            asesors.forEach((asesor) => {
              if (asesor.programStudiId !== undefined) {
                const programStudiId = asesor.programStudiId;
                const asesorIdLapangan = asesor.asesorId;
                localStorage.setItem("programStudiId", programStudiId);
                localStorage.setItem("asesorIdLapangan", asesorIdLapangan);
              }
            });
          }
        }
                ...
                return (
                  ...
                  >
                    {tableLoading && isActive ? (
                      <>
                        <span role="img" aria-label={text} style={{ fontSize: '19px' }}>{icon}</span>
                        <span className="text d-flex gap-1" style={{ fontSize: '14px' }}>
                          {isScreenSizeLowerThanLg
                            ? (function () {
                              // ... (switch case remains unchanged)
                            })()
                            : text}
                          <CircularProgress className="mt-1" size={7} color="success" />
                        </span>
            ...
            <ContentCard>
              ...
              <div className="col-12">
                <div className={`row d-flex flex-sm-row flex-column ${isScreenSizeLowerThanMD ? 'gap-3' : 'align-items-center'}`} style={{ padding: "20px" }}>
                    ...
                    {view === 0 && (
                      <div className="me-3">
                        ...
                        >
                          ...
                          <option value={19}>Dokumen Dikembalikan</option>
                        </select>
                      </div>
                    )}
                    {view === 2 && (
                      <div className="me-3">
                        ...
                        >
                          ...
                          <option value={20}>Penawaran Asessment</option>
                  ...
                  {view === 5 && selectedAsesorIdsToSkInProgress.length > 0 && (
