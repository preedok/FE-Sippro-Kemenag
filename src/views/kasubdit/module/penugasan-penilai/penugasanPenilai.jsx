...
const BerandaKasubdit = () => {
  ...
  const handleSearchQueryChange = (event) => {
    ...
    const filteredPenawaranRows = rows.filter((row) => {
      const asesorLowerCase = typeof row.namaProdi === 'string' ? row.namaProdi.toLowerCase() : '';
      const jenjangLowerCase = typeof row.jenjang === 'string' ? row.jenjang.toLowerCase() : '';
      const lembagaLowerCase = typeof row.lembaga === 'string' ? row.lembaga.toLowerCase() : '';
      const noregLowerCase = typeof row.nomorRegistrasi === 'string' ? row.nomorRegistrasi.toLowerCase() : '';
      const asesorFullName = typeof row.asesorFullName === 'string' ? row.asesorFullName.toLowerCase() : '';
      const asesorEmail = typeof row.asesorEmail === 'string' ? row.asesorEmail.toLowerCase() : '';
      ...
      return (
        ...
        noregLowerCase.includes(query.toLowerCase()) ||
        asesorFullName.includes(query.toLowerCase()) ||
        asesorEmail.includes(query.toLowerCase())
      );
    });
    setFilteredRows(filteredPenawaranRows);
    console.log('search ak', filteredPenawaranRows);
  };
  // Penawaran AK
  ...
  const getPenawaran = async () => {
    ...
    try {
      ...
      console.log('get list prodi', penawaranRows);
    } catch (error) {
  ...
  const getPenawaranAL = async () => {
    ...
    try {
      ...
      console.log('get list prodi al', penawaranRowsAl);
    } catch (error) {
      console.log("Error fetching Penawaran data:", error);
    } finally {
      setLoading(false);
  ...
  const handleSearchQueryChangeAl = (event) => {
    ...
    const filteredPenawaranRowsAl = rowsAl.filter((row) => {
      const asesorLowerCase = typeof row.namaProdi === 'string' ? row.namaProdi.toLowerCase() : '';
      const jenjangLowerCase = typeof row.jenjang === 'string' ? row.jenjang.toLowerCase() : '';
      const lembagaLowerCase = typeof row.lembaga === 'string' ? row.lembaga.toLowerCase() : '';
      const noregLowerCase = typeof row.nomorRegistrasi === 'string' ? row.nomorRegistrasi.toLowerCase() : '';
  ...
  //     const asesorLowerCase = row.namaProdi.toLowerCase();
  //     const jenjangLowerCase = String(row.jenjang).toLowerCase();
  //     const lembagaLowerCase = String(row.lembaga).toLowerCase();
  //     const noregLowerCase = String(row.nomorRegistrasi).toLowerCase();
  //     const asesorFullName = String(row.asesorFullName).toLowerCase();
  //     const asesorEmail = String(row.asesorEmail).toLowerCase();
  //     return (
  //       asesorLowerCase.includes(query.toLowerCase()) ||
  //       jenjangLowerCase.includes(query.toLowerCase()) ||
  //       lembagaLowerCase.includes(query.toLowerCase()) ||
  ...
  const handleSearchQueryChange1 = (event) => {
    const query = event.target.value;
    setSearchQuery1(query);
    const filteredPenawaranRows1 = rows1.filter((row) => {
      const asesorLowerCase = typeof row.namaProdi === 'string' ? row.namaProdi.toLowerCase() : '';
      const jenjangLowerCase = typeof row.jenjang === 'string' ? row.jenjang.toLowerCase() : '';
      const lembagaLowerCase = typeof row.lembaga === 'string' ? row.lembaga.toLowerCase() : '';
      const noregLowerCase = typeof row.nomorRegistrasi === 'string' ? row.nomorRegistrasi.toLowerCase() : '';
    ...
    setFilteredRows1(filteredPenawaranRows1);
    console.log('search daftar penilai', filteredPenawaranRows1);
  };
  const columns1 = [
    { id: "no", label: "#", minWidth: 40 },
  ...
  const getPenilai = async () => {
    ...
    try {
      ...
      console.log('get list prodi daftar penilai', penilaiRows1);
    } catch (error) {
