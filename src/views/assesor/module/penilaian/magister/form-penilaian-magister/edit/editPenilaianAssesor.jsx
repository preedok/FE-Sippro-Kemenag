...
import BobotEnumDoctor from '../../../utils/BobotS2'
...
const style = {
  ...
  const getData = async () => {
    ...
    setTimeout(async () => {
      try {
        ...
        setAsesorEvaluator(response.data.data.asesorProgramStudis.asesorEvaluator)
        setAsesorProdiAkreditasi(response.data.data.asesorProgramStudis.asesorProdiAkreditasi)
        setAsesorProdiAsal(response.data.data.asesorProgramStudis.asesorProdiAsal)
  ...
  const [rows1, setRows1] = useState([
    ...
    },
    {
      ...
      note: '',
      keterangan:
        "Jumlah dan kualifikasi tenaga kependidikan: Jumlah minimal tenaga kependidikan terdiri atas 2(dua) orang tenaga kependidikan untuk setiap program studi dan 1(satu) orang untuk melayani perpustakaan.Kualifikasi tenaga kependidikan minimal berijazah Diploma Tiga, berusia maksimum 56 tahun, dan bekerja penuh waktu 37, 5 jam / minggu ",
      nilai: "",
  ...
  const [rows11, setRows11] = useState([
    ...
    },
    {
      butir: "1.5 & 1.6",
      keterangan:
        "Ketersediaan RPS tes(Rencana Pembelajaran Semester) untuk 5 (lima) matakuliah penciri program studi yang diusulkan yang memenuhi 9 (sembilan) komponen:  1. Nama program studi, nama dan kode mata kuliah, semester, sks, nama dosen pengampu;, 2. Capaian Pembelajaran lulusan yang dibebankan pada mata kuliah;, 3. Kemampuan akhir yang direncanakan pada tiap tahap pembelajaran untuk memenuhi capaian pembelajaran lulusan;, 4. Bahan kajian yang terkait dengan kemampuan yang akan dicapai, 5. Metode pembelajaran; 6. Waktu yang disediakan untuk mencapai kemampuan pada tiap tahap pembelajaran;, 7. Pengalaman belajar mahasiswa yang diwujudkan dalam deskripsi tugas yang harus dikerjakan oleh mahasiswa selama satu semester;, 8. Kriteria, indikator, dan bobot penilaian; dan 9. Daftar referensi yang digunakan.",
      nilai2: "",
      showButton4: true,
    },
    {
  ...
  const handleNilaiChangeTable11 = (e, index) => {
    ...
    const finalValue = calculateFinalValue1();
    const updatedRows11 = rows11.map((row, rowIndex) => {
      if (rowIndex === index) {
        return {
          ...row,
          nilai2: value === finalValue ? finalValue : value,
        };
      }
      return row;
    });
  ...
  const handleNChange2 = (e, field) => {
    ...
    setFormData2((prevFormData) => {
      ...
      if (field === 'ltrk2' || field === 'lt2') {
        const luasRuangPerMahasiswa = calculateLuasRuangPerMahasiswa(updatedFormData.ltrk2, updatedFormData.lt2);
        return {
          ...updatedFormData,
          luasRuangPerMahasiswa,
        };
  ...
  const handleNChange3 = (e, field) => {
    ...
    setFormData3((prevFormData) => {
      ...
      if (field === 'ltrk3' || field === 'lt3') {
        const luasRuangPerMahasiswa = calculateLuasRuangPerMahasiswa3(updatedFormData.ltrk3, updatedFormData.lt3);
        return {
          ...updatedFormData,
          luasRuangPerMahasiswa,
        };
  ...
  const handleNChange4 = (e, field) => {
    ...
    setFormData4((prevFormData) => {
      ...
      if (field === 'ltrk4' || field === 'lt4') {
        const luasRuangPerMahasiswa = calculateLuasRuangPerMahasiswa4(updatedFormData.ltrk4, updatedFormData.lt4);
        return {
          ...updatedFormData,
          luasRuangPerMahasiswa,
        };
  ...
  //validasi lapangan
  const validateForm1 = () => {
    for (const row of rows10) {
      if (!row.butir || !row.keterangan) {
        return false;
      }
    }

    for (const row of rows11) {
      // Skip the validation if the value is coming from calculateFinalValue()
      if (!row.note2 && !(row.nilai2 && row.nilai2.toString() == calculateFinalValue1())) {
        return false;
      }
    }
    return true;
  ...
  // create PUT penilaian lapangan
  const handlePutRequest11 = async () => {
    ...
    try {
        ...
        questions: {
          ...
          form2: rows11.map((row) => ({
            ...
            additionalFields2: {
              ...
              Nc5: formData5.Nc5,
              f5: formData5.f5,
              ltrk6: formData6.ltrk6,
              lt6: formData6.lt6,
              lrkpm6: formData6.lrkpm6,
              status6: formData6.status6,
              ltrk7: formData7.ltrk7,
              lt7: formData7.lt7,
              lrkpm7: formData7.lrkpm7,
              status7: formData7.status7,
              ltrk8: formData8.ltrk8,
              lt8: formData8.lt8,
              lrkpm8: formData8.lrkpm8,
              status8: formData8.status8,
            },
          })),
        },
      };
      formPenilaian.questions.form1 = formPenilaian.questions.form1.map(
        (row) => {
          if (row.code === "*") {
            return {
              ...row,
              nilai2: isAllAda1 ? "MEMENUHI" : "TIDAK MEMENUHI",
            };
          }
          return row;
        }
      ...
      const response = await api.put(
        "prodi-assesment/assesment/lapangan",
        formPenilaian,
        {
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      if (response && response.data) {
        ...
        console.error(
          "Error sending PUT request: Response or data is undefined."
        );
        Swal.fire({
          icon: "error",
          title: "Error",
      ...
      if (error.response && error.response.data) {
        ...
        Swal.fire({
          icon: "error",
          title: "Error",
          text: `Error sending PUT request: ${error.message}`,
        });
      } else {
        console.error("Error sending PUT request:", error.message);
        Swal.fire({
      ...
      const calculatedValues = rows1.map((row) => {
        ...
        const parsedNilai =
          ...
          (data?.asesorProgramStudis?.questions?.form2 &&
            ...
            )) || parseFloat(row.nilai) || (data?.asesorProgramStudis?.questions?.form2 &&
              data.asesorProgramStudis.questions.form2.find(
    ...
    try {
      ...
      const formPenilaian = {
        ...
        questions: {
          ...
          form2: rows1.map((row) => ({
            ...
            note: row.note,
            additionalFields: {
              Na: String(formData.Na),
              Nb: String(formData.Nb),
              Nc: String(formData.Nc),
              f: String(formData.f),
              ltrk2: String(formData2.ltrk2),
              lt2: String(formData2.lt2),
              lrkpm2: String(formData2.luasRuangPerMahasiswa),
              status2: String(formData2.status2),
              ltrk3: String(formData3.ltrk3),
              lt3: String(formData3.lt3),
              lrkpm3: String(formData3.luasRuangPerMahasiswa),
              status3: String(formData3.status3),
              ltrk4: String(formData4.ltrk4),
              lt4: String(formData4.lt4),
              lrkpm4: String(formData4.luasRuangPerMahasiswa),
              status4: String(formData4.status4),
              ltrp: String(formData9.ltrp)
            },
          })),
        },
        asesorEvaluator: typeof asesorEvaluator === "string" ? asesorEvaluator : "",
        asesorPtAsal: typeof asesorPtAsal === "string" ? asesorPtAsal : "",
        asesorProdiAsal: typeof asesorProdiAsal === "string" ? asesorProdiAsal : "",
        asesorProdiAkreditasi: typeof asesorProdiAkreditasi === "string" ? asesorProdiAkreditasi : "",
        komentar: typeof coment === "string" ? coment : "",
        totalNilai: Number(totalSum),
      ...
      const form2NilaiIsInvalid = formPenilaian.questions.form2.some(row => {
        ...
        if (parsedNilai > 4) {
          ...
          throw new Error('Nilai tidak boleh lebih dari 4. Harap periksa kembali.');
        }
  ...
  function handleKeyPress3(e) {
    ...
    if (parseFloat(inputValue) === 3) {
      alert('Nilai tidak boleh sama dengan 3.');
      e.preventDefault();
      return;
    }
  }
      ...
      >
        ...
        <div className="card-body">
          ...
          <div className="row container mt-4" style={{ lineHeight: "5px" }}>
            ...
            <div className="col-sm-12 col-md-9" >
              <TextField value={asesorEvaluator || ''}
                onChange={(e) => setAsesorEvaluator(e.target.value)} style={{ height: '5px', marginTop: '-15px' }} size="small" variant="outlined" />
              {!asesorEvaluator && !data?.asesorProgramStudis?.asesorEvaluator && (
                <p
                  className="text-danger"
                  style={{ fontStyle: "italic", marginTop: '35px', marginBottom: '30px' }}
                >
                  Belum di Isi
                </p>
              )}
            ...
            <div className="col-sm-12 col-md-9 mb-1" >
              <TextField value={asesorPtAsal || ''}
                onChange={(e) => setAsesorPtAsal(e.target.value)} style={{ height: '5px', marginTop: '-15px' }} size="small" variant="outlined" />
              {!asesorPtAsal && !data?.asesorProgramStudis?.asesorPtAsal && (
                <p
                  className="text-danger"
                  style={{ fontStyle: "italic", marginTop: '35px', marginBottom: '30px' }}
                >
            ...
            <div className="col-sm-12 col-md-9 mb-1" >
              <TextField value={asesorProdiAsal || ''}
                onChange={(e) => setAsesorProdiAsal(e.target.value)} style={{ height: '5px', marginTop: '-15px' }} size="small" variant="outlined" />
              {!asesorProdiAsal && !data?.asesorProgramStudis?.asesorProdiAsal && (
                <p
                  className="text-danger"
                  style={{ fontStyle: "italic", marginTop: '35px', marginBottom: '30px' }}
                >
            ...
            <div className="col-sm-12 col-md-9 mb-1" >
              <TextField value={asesorProdiAkreditasi || ''}
                onChange={(e) => setAsesorProdiAkreditasi(e.target.value)} style={{ height: '5px', marginTop: '-15px' }} size="small" variant="outlined" />
              {!asesorProdiAkreditasi && !data?.asesorProgramStudis?.asesorProdiAkreditasi && (
                <p
                  className="text-danger"
                  style={{ fontStyle: "italic", marginTop: '35px', marginBottom: '30px' }}
                >
          ...
          <div className="row mt-3">
            {view === "one" ? (
                      ...
                      <TableBody style={{
                        ...
                        {isSubmitted ? (
                          ...
                          rows.map((row, index) => {
                            ...
                            return (
                              <TableRow key={index}>
                                <TableCell style={{ color: darkMode ? "white" : "", }} scope="row">{row.butir}</TableCell>
                                <TableCell style={{ color: darkMode ? "white" : "", }}>{row.keterangan}</TableCell>
                                    ...
                                    >
                                      <span>
                                        {data?.asesorProgramStudis?.questions?.form1?.find(item => item.code === '*')?.nilai ?
                                          data.asesorProgramStudis.questions.form1.find(item => item.code === '*').nilai :
                                          (isAllAda ? "MEMENUHI" : "TIDAK MEMENUHI")}
                                      </span>
                                    </button>
                                  ) : (
                                    <>
                                ...
                                {index === 1 && (
                                  <TableCell style={{ color: darkMode ? "white" : "", }} rowSpan={rows.length}>
                                    <textarea
                          ...
                          rows1.map((row, index) => (
                            <TableRow style={{
                              ...
                              <TableCell style={{ color: darkMode ? "white" : "", }}>
                                <div className="d-flex">
                                  ...
                                  {row.showButton7 && (
                                    <>
                                      ...
                                      <div>
                                        <input
                                          value={calculateFinalValue() || rows1[index].nilai}
                                          className="form-control"
                                          type="text"
                                          disabled
                                        />
                                    ...
                                    <div>
                                      ...
                                      <div className="d-flex">
                                        <button
                                          ...
                                          className="text-center btn btn-success rounded-5 fw-bold me-3 mt-3"
                                        >
                                          <EditNotificationsIcon />
                                        </button>
                                        <input
                                  ...
                                  {row.showButton16 && (
                                    <>
                                      ...
                                      <input
                                        ...
                                        onKeyPress={(e) => handleKeyPress3(e)}
                                        onChange={(e) =>
                                          handleNilaiChangeTable2(e, index)
                                        }
                                        className="form-control"
                                  ...
                                  (
                                    <p
                                      ...
                                      style={{
                                        color: "red",
                                        fontStyle: "italic",
                                        display: row.butir === '3.3.1' || row.butir === '2.2' ? 'none' : 'block'
                                      }}
                                    >
                                      Belum di Isi
                                    </p>
                                ...
                                {calculateFinalValue() == 0 && (
                                  <p
                                    className="mt-2 text-center"
                                    style={{
                                      color: "red",
                                      fontStyle: "italic",
                                      display: row.butir === '2.2' ? 'block' : 'none'
                                    }}
                                  >
                                    Belum di Isi
                                  </p>
                              ...
                              <TableRow style={{
                                ...
                                <TableCell style={{ color: darkMode ? "white" : "", }}>
                                  <div className="d-flex">
                                    ...
                                    {row.showButton7 && (
                                      <>
                                        {rows1[index].nilai || calculateFinalValue()}
                                      </>
                                    )}
                                    {row.showButton8 && (
                                      <>
                                ...
                                <TableCell style={{ color: darkMode ? "white" : "", }} className="text-center">{row.bobot}</TableCell>
                                <TableCell style={{ color: darkMode ? "white" : "", }} className="text-center">{terbobotiValues[index] || 'Loading...'}</TableCell>
                              </TableRow>
                            );
                          })
                    ...
                    {isSubmitted ? (
                      ...
                      rows4.map((row, index) => (
                        <TableRow style={{
                          ...
                          <TableCell style={{ color: darkMode ? "white" : "", }} scope="row">{index + 1}</TableCell>
                          <TableCell style={{ color: darkMode ? "white" : "", }}>{row.dokumen}</TableCell>
                          <TableCell style={{ color: darkMode ? "white" : "", }}>
                            {!row.status && ('-')}
                            {row.status == 1 && (<CheckCircleIcon color="green" />)}
                            {row.status == 9 && (<XLgIcon color="red" />)}
                          </TableCell>
                          <TableCell style={{ color: darkMode ? "white" : "", }}>{row.notes ? row.notes : '-'}</TableCell>
                          <TableCell style={{ color: darkMode ? "white" : "", }}>{row.file}</TableCell>
                        </TableRow>
                      ))
              ...
              <>
                ...
                <div className="d-flex">
                  ...
                  <select onChange={handleChange} defaultValue="konfirmasi" className="form-select ms-2" aria-label="Default select example">
                            ...
                            <TableCell>
                              <div className="d-flex">
                                ...
                                {row.showButton7 && (
                                  <>
                                    <button
                                      onClick={() => handleOpen30(index)}
                                      className="text-center btn btn-success rounded-5 fw-bold me-3 mt-1"
                                    >
                                      <EditNotificationsIcon />
                                    </button>
                                    <input
                                      value={rows11[index].nilai2 || calculateFinalValue1()}
                                      onChange={(e) =>
                                        handleNilaiChangeTable11(e, index)
                                      }
                                      className="form-control"
            ...
            <div className="col-12 mt-3">
              <h5>Rekomendasi <span class="text-danger fst-italic fs-6">*Wajib Isi</span></h5>
              <div className="form-check">
                ...
                />
                <label className="form-check-label" htmlFor="recommendationCheckbox">
                  Direkomendasikan
                </label>
              </div>
              ...
              <div className="form-check">
                <input
                  ...
                  onChange={handleCheckboxChange}
                />
                <label className="form-check-label" htmlFor="notRecommendedCheckbox">
            ...
            <div className="col-12 mt-1">
              <Alert severity="warning">Pastikan nilai dan catatan sudah terisi semua sebelum submit</Alert>
            </div>
            <div className="col-12 mt-4 d-flex">
              ...
              <div className="d-flex gap-3 ms-auto">
                {/* <Button
                  disabled={data?.asesorProgramStudis?.status === 23 || isSubmitted}
                  sx={{
                    transition: "transform 0.3s ease-in-out",
                    backgroundColor: "red",
                    "&:hover": {
                  ...
                  variant="contained"
                  onClick={() => {
                    InputSwal2('Catatan Pengembalian', 'Placeholder...', async (note) => {
                      if (note) {
                        try {
                          const id = parseInt(getUserId());
                          const programStudiId = parseInt(localStorage.getItem('programStudiId'));
                          const apiUrl = `/prodi-assesment/reject/${programStudiId}/${id}?note=${encodeURIComponent(note)}`;
                          ...
                          if (error.response) {
                            if (error.response.status === 400 && error.response.data.errors?.note) {
                              ...
                              await Swal.fire({
                                icon: 'error',
                                title: 'Gagal',
                                text: error.response.data.title || 'Terjadi kesalahan',
                              });
                            }
                          } else {
                            await Swal.fire({
                ...
                <Button
                  ...
                  sx={{
                    transition: "transform 0.3s ease-in-out",
                    backgroundColor: `${data?.asesorProgramStudis?.status === 23 ? 'green' : 'blue'}`,
                    "&:hover": {
                      transform: "translateY(-5px)",
                      backgroundColor: "transparent",
                      color: "blue",
                ...
                >
                  {data?.asesorProgramStudis?.status === 23 ? 'Edit Penilaian' : 'Submit Penilaian Kecukupan'}
                </Button>
              </div>
            ...
            <Box sx={style}>
              ...
              <Typography id="modal-modal-description" sx={{ mt: 2 }}>
                <Alert severity="info">
                  ...
                  perbandingan tiga program studi pada tingkat internasional yang
                  mencakup tiga aspek, atau prodi yang diusulkan merupakan
              ...
              <Typography id="modal-modal-description" sx={{ mt: 2 }}>
                <Alert severity="info">
                  ...
                  perbandingan kurang dan tiga program studi pada tingkat nasional
                  dan/atau mencakup kurang dari tiga aspek
                </Alert>
              </Typography>
              <Typography id="modal-modal-description" sx={{ mt: 2 }}>
                <Alert severity="info">
            ...
            <Box sx={style}>
              ...
              <Typography id="modal-modal-description" sx={{ mt: 2 }}>
                <Alert severity="info">
                  ...
                  dengan Program Magister dan (2) keterkaitan profil dengan
                  keunikan atau keunggulan prodi
                </Alert>
              </Typography>
              <Typography id="modal-modal-description" sx={{ mt: 2 }}>
                <Alert severity="info">
                  ...
                  sesuai dengan Program Magister dan (2) keterkaitan profil dengan
              ...
              <Typography id="modal-modal-description" sx={{ mt: 2 }}>
                <Alert severity="info">
                  ...
                  profesi atau jenis pekerjaan atau bentuk kerja lainnya dan
                  keterkaitan profil dengan keunggulan atau keunikan program studi
                </Alert>
              </Typography>
              <Typography id="modal-modal-description" sx={{ mt: 2 }}>
                <Alert severity="info">
            ...
            <Box sx={style}>
              ...
              <Typography id="modal-modal-description" sx={{ mt: 2 }}>
                <Alert severity="info">
                  ...
                  KKNI dan sesuai SN-Dikti yang mencakup 4 (empat) domain capaian
                  pembelajaran, (3) relevan dengan keunikan atau keunggulan prodi,
                  dan (4)
              ...
              <Typography id="modal-modal-description" sx={{ mt: 2 }}>
                <Alert severity="info">
                  ...
                  lulusan, (b) deskripsi kompetensinya sesuai level 8 (delapan)
                  KKNI dan sesuai SN-Dikti yang mencakup 4 (empat) domain capaian
                  pembelajaran, dan (3)relevan dg keunggulan atau keunikan prodi
                </Alert>
              </Typography>
              <Typography id="modal-modal-description" sx={{ mt: 2 }}>
                <Alert severity="info">
            ...
            <Box sx={style}>
              ...
              <Typography id="modal-modal-description" sx={{ mt: 2 }}>
                <Alert severity="info">
                  ...
                  4 Lima mata kuliah dilengkapi dengan RPS yang memenuhi 9 komponen, menunjukkan secara jelas penciri program studi dan menggunakan referensi yang relevan dan mutakhir
                </Alert>
              </Typography>
              <Typography id="modal-modal-description" sx={{ mt: 2 }}>
                <Alert severity="info">

                  3 Lima mata kuliah dilengkapi dengan RPS yang memenuhi 9 komponen, menunjukkan secara jelas penciri program studi dan menggunakan referensi yang relevan
                </Alert>
              </Typography>
              <Typography id="modal-modal-description" sx={{ mt: 2 }}>
                <Alert severity="info">
                  2 Lima mata kuliah dilengkapi dengan RPS yang memenuhi 9 komponen
                </Alert>
              </Typography>
              <Typography id="modal-modal-description" sx={{ mt: 2 }}>
                <Alert severity="info">
            ...
            <Box sx={style}>
              ...
              <Typography id="modal-modal-description" sx={{ mt: 2 }}>
                <Alert severity="info">
                  ...
                  4 Jumlah calon dosen tetap sedikitnya sebanyak 5 (lima) orang berkualifikasi akademik lulusan doktor atau doktor terapan atau setara level 9 (sembilan) KKNI, dan telah memiliki NIDN pada PT pengusul
                </Alert>
              </Typography>
              <Typography id="modal-modal-description" sx={{ mt: 2 }}>
                <Alert severity="info">
                  ...
                  berkualifikasi akademik lulusan doktor atau doktor terapan atau
                  setara level 9 (sembilan) KKNI, dengan komposisi: (a) 3 (tiga)
                  orang dosen tetap dari PT pengusul yang telah memiliki NIDN, dan
                  (b) 2 (dua) orang Guru Besar, dan telah menandatangani Surat
                  Perjanjian Kesediaan Pengangkatan sebagai calon dosen tetap
                </Alert>
              </Typography>
              <Typography id="modal-modal-description" sx={{ mt: 2 }}>
                <Alert severity="info">
                  ...
                  berkualifikasi akademik lulusan doktor atau doktor terapan atau
                  setara level 9 (sembilan) KKNI, dengan komposisi: (a) 3 (tiga)
                  orang dosen tetap dari PT pengusul yang telah memiliki NIDN atau
                  telah menandatangani Surat Perjanjian Kesediaan Pengangkatan
                  sebagai calon dosen tetap, dan (b) 2 (dua) orang lainnya
                  merupakan dosen dari PT lain yang ditugaskan oleh Pemimpin PT
                </Alert>
              </Typography>
              <hr />
              <div className="mt-3 justify-content-end d-flex">
            ...
            <Box sx={style}>
              ...
              <div className="d-flex">
                <Typography id="modal-modal-description" sx={{ mt: 2 }}>
                  ...
                  ilmiah/seni/olah raga pada jurnal internasional bereputasi, yang
                  dihasilkan dari penelitian dan pengabdian kepada masyarakat
                </Typography>
                <div className=" ms-auto">
                  ...
                  />
                  {!formData.Na && (!data?.asesorProgramStudis?.questions?.form2?.[2]?.additionalFields?.Na) && (
              ...
              <div className="d-flex">
                ...
                <div className=" ms-auto">
                  ...
                  />
                  {!formData.Nb && (!data?.asesorProgramStudis?.questions?.form2?.[2]?.additionalFields?.Nb) && (
                    <p className="text-danger">Nilai Belum Ada</p>
                  )}
                </div>
              </div>
              ...
              <div className="d-flex">
                <Typography id="modal-modal-description" sx={{ mt: 2 }}>
                  ...
                  ilmiah/seni/olah raga pada jurnal nasional, yang dihasilkan dari
                ...
                <div className=" ms-auto">
                  ...
                  />
                  {!formData.Nc && (!data?.asesorProgramStudis?.questions?.form2?.[2]?.additionalFields?.Nc) && (
                    <p className="text-danger">Nilai Belum Ada</p>
                  )}
                </div>
              </div>
              ...
              <div className="d-flex">
                ...
                <div className=" ms-auto">
                  ...
                  />
                  {!formData.f && (!data?.asesorProgramStudis?.questions?.form2?.[2]?.additionalFields?.f) && (
                    <p className="text-danger">Nilai Belum Ada</p>
                  )}
                </div>
              </div>

              <Typography sx={{ marginTop: '20px', fontWeight: 'bold', fontSize: '20px' }}>Nilai Kasar (NK) = {calculateNK()}</Typography>
              <Typography sx={{ marginTop: '20px', fontWeight: 'bold', fontSize: '20px' }}>
                Nilai = {calculateFinalValue()}
              </Typography>
              <hr />
              <div className="mt-3 justify-content-end d-flex">
                  ...
                  <input
                    ...
                    value={
                      ...
                      (data?.asesorProgramStudis?.questions &&
                        data.asesorProgramStudis.questions.form2 &&
                        data.asesorProgramStudis.questions.form2[2]?.additionalFields2?.Na5) ||
                      ''
                    }
                    onChange={(e) => handleNChange5(e, "Na5")}

                  />
                  {!formData5.Na5 && (!data?.asesorProgramStudis?.questions?.form2?.[2]?.additionalFields2?.Na5) && (
                    <p className="text-danger">Nilai Belum Ada</p>
                  )}
                </div>
              </div>
                ...
                <div className=" ms-auto">
                  <input
                    ...
                    value={
                      ...
                      (data?.asesorProgramStudis?.questions &&
                        data.asesorProgramStudis.questions.form2 &&
                        data.asesorProgramStudis.questions.form2[2]?.additionalFields2?.Nb5) ||
                      ''
                    }
                    onChange={(e) => handleNChange5(e, "Nb5")}

                  />
                  {!formData5.Nb5 && (!data?.asesorProgramStudis?.questions?.form2?.[2]?.additionalFields2?.Nb5) && (
                    <p className="text-danger">Nilai Belum Ada</p>
                  )}
                </div>
              </div>
                ...
                <div className=" ms-auto">
                  <input
                    ...
                    value={
                      ...
                      (data?.asesorProgramStudis?.questions &&
                        data.asesorProgramStudis.questions.form2 &&
                        data.asesorProgramStudis.questions.form2[2]?.additionalFields2?.Nc5) ||
                      ''
                    }
                    onChange={(e) => handleNChange5(e, "Nc5")}

                  />
                  {!formData5.Nc5 && (!data?.asesorProgramStudis?.questions?.form2?.[2]?.additionalFields2?.Nc5) && (
                    <p className="text-danger">Nilai Belum Ada</p>
                  )}
                </div>
              </div>
                ...
                <div className=" ms-auto">
                  <input
                    ...
                    value={
                      ...
                      (data?.asesorProgramStudis?.questions &&
                        data.asesorProgramStudis.questions.form2 &&
                        data.asesorProgramStudis.questions.form2[2]?.additionalFields2?.f5) ||
                      ''
                    }
                    onChange={(e) => handleNChange5(e, "f5")}

                  />
                  {!formData5.f5 && (!data?.asesorProgramStudis?.questions?.form2?.[2]?.additionalFields2?.f5) && (
                    <p className="text-danger">Nilai Belum Ada</p>
                  )}
                </div>
              </div>

              <Typography sx={{ marginTop: '20px', fontWeight: 'bold', fontSize: '20px' }}>Nilai Kasar (NK) = {calculateNK1()}</Typography>
              <Typography sx={{ marginTop: '20px', fontWeight: 'bold', fontSize: '20px' }}>
            ...
            <Box sx={style}>
              ...
              <div className="d-flex">
                ...
                <div className=" ms-auto">
                  ...
                  />
                  {!formData9.ltrp && (!data?.asesorProgramStudis?.questions?.form2?.[4]?.additionalFields?.ltrp) && (
                    <p className="text-danger">Nilai Belum Ada</p>
                  )}
                </div>
              </div>
            ...
            <Box sx={style}>
              ...
              <Typography id="modal-modal-description" sx={{ mt: 2 }}>
                <Alert severity="info">
                  ...
                  4 Memiliki ruang belajar mandiri lebih dari 4m2/mahasiswa, milik
                  sendiri, dilengkapi perabot kantor dan internet
                </Alert>
              </Typography>
              <Typography id="modal-modal-description" sx={{ mt: 2 }}>
                <Alert severity="info">
            ...
            <Box sx={style}>
              ...
              <Typography id="modal-modal-description" sx={{ mt: 2 }}>
                <Alert severity="info">
                  ...
                  4 Peralatan tersedia dalam jumlah yang mencukupi dan sesuai dengan mata kuliah berpraktikum/ berpraktik  untuk pembelajaran sampai 2 (dua) tahun pembelajaran
                </Alert>
              </Typography>
              <Typography id="modal-modal-description" sx={{ mt: 2 }}>
                <Alert severity="info">
              ...
              <Typography id="modal-modal-description" sx={{ mt: 2 }}>
                <Alert severity="info">
                  2 Peralatan tersedia dalam jumlah dan mutu yang mencukupi untuk pembelajaran pada 1 (satu) tahun pertama
                </Alert>
              </Typography>
              <Typography id="modal-modal-description" sx={{ mt: 2 }}>
                <Alert severity="info">

                  1 Peralatan tersedia dalam jumlah yang mencukupi untuk pembelajaran, tetapi tidak sesuai dengan mata kuliah berpraktikum
                </Alert>
              </Typography>
              <Typography id="modal-modal-description" sx={{ mt: 2 }}>
                <Alert severity="info">
            ...
            <Box sx={style}>
              ...
              <Typography id="modal-modal-description" sx={{ mt: 2 }}>
                <Alert severity="info">
                  ...
                  4 Jika jumlah tenaga kependidikan lebih dari 3 (tiga) orang dan
                  salah satu diantaranya berkualifikasi magister dan 1 (satu)
              ...
              <Typography id="modal-modal-description" sx={{ mt: 2 }}>
                <Alert severity="info">
                  ...
                  3 Jika jumlah tenaga kependidikan lebih dari 2 (dua) orang atau
                  berkualifikasi sarjana atau sarjana terapan dan 1 (satu) orang
                  pustakawan ditingkat perguruan tinggi dengan kualifikasi Diploma
                  Tiga perpustakaan atau yang sejenis
                </Alert>
              </Typography>
              <Typography id="modal-modal-description" sx={{ mt: 2 }}>
                <Alert severity="info">
